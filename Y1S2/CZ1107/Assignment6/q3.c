...
{
    if((Q1Ptr->Table[Hash(key,Q1Ptr->hSize)]).head == NULL){
      ListNode *temp = malloc(sizeof(ListNode));
      temp->key = key;
      temp->pre = NULL;
      temp->next = NULL;
      (Q1Ptr->Table[Hash(key,Q1Ptr->hSize)]).head = temp;
      return 1;
    }
    else{
      ListNode *temp1;
      temp1 = (Q1Ptr->Table[Hash(key,Q1Ptr->hSize)]).head;
      while (temp1 != NULL){
        if(temp1->key == key){
          return 0;
        }
      ...
      temp2->pre = NULL;
      temp2->key = key;
      temp2->next = ((Q1Ptr->Table[Hash(key,Q1Ptr->hSize)]).head);
      ((Q1Ptr->Table[Hash(key,Q1Ptr->hSize)]).head)->pre = temp2;
      ((Q1Ptr->Table[Hash(key,Q1Ptr->hSize)]).head) = temp2;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    while(temp1 != NULL){
      if(temp1->key == key){
        if((temp1 -> pre == NULL)&&(temp1 -> next == NULL)){
            ((Q1Ptr->Table[Hash(key,Q1Ptr->hSize)]).head) = NULL;
            free(temp1);
            return 1;
        }
        else if (temp1 -> pre == NULL){
          ((Q1Ptr->Table[Hash(key,Q1Ptr->hSize)]).head) = temp1->next;
          free(temp1);
          return 1;
        }
